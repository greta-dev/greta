...
tf_cpu_deps <- tf_deps |>
  ...
  select(os, version, python_version) |>
  mutate(
...
greta_deps_tf_tfp <- .deps_tf |>
  left_join(.deps_tfp, by = "tf_version", relationship = "many-to-many") |>
  relocate(tfp_version, .after = tf_version) |>
  mutate(
