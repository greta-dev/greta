...
* `greta_deps_spec()` now accepts TensorFlow from 2.18.0 up to 2.21.0, and rejects anything outside that range rather than only rejecting versions that are too new. TensorFlow 2.16 and 2.17 ship Keras 3 and install without complaint, but cannot load TensorFlow Probability, because greta pins TFP to the 0.25 series whatever the TensorFlow version; you would previously have found that out as a Python import error after a full install, and now get an R error naming the supported range (#633, #638).
