...
* `greta_set_deps()` persistently chooses which TensorFlow, TensorFlow Probability, and Python versions greta uses; the managed (uv) environment installs them on next load and `install_greta_deps()` uses them as its default; clear with `greta_remove()` (#817).
...
* `greta_sitrep()` now reports the resolved Python backend and whether greta can start offline: for the managed (uv) environment, the `UV_OFFLINE` setting and whether reticulate's uv cache is populated (#801, #817).
