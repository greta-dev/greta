...
* `calculate()` with `nsim` no longer leaves R's random number stream reseeded. It seeds R as well as TensorFlow, and only the branch taking an explicit `seed` put the stream back afterwards, so an unseeded call changed every random draw that followed it in the session. It now restores the stream either way, having advanced it by the one draw it takes to pick a seed - the same contract `mcmc()` keeps (#285).
* `mcmc()` now respects `set.seed()`. It always drew its own seed from R's RNG, but then stored it without ever passing it to TensorFlow, so a run was only reproducible when something else in the session had already set TensorFlow's global seed - a previous `calculate()` call, for example. `set.seed()` and `tensorflow::set_random_seed()` now give identical results, as `?mcmc` has always said they would. Draws taken with a fixed seed therefore differ from previous versions (#285, #427).
