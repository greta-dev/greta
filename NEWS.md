...
* `opt()` with `hessian = TRUE` is faster, and no longer prints TensorFlow's "triggered tf.function retracing" warning for models whose targets are small. It used to rebuild the model's TensorFlow graph once per target greta array, and to take every hessian with TensorFlow's vectorised `pfor`, which traces a fresh function on each call. greta now builds the graph once for all of them, and uses `pfor` only for targets of at least 100 elements, where vectorising pays for itself. On a model with twenty scalar targets this measured at 12.1 seconds before and 1.2 after. Models with large targets are as fast as before and can still emit the warning, since they still use `pfor` (#546).
