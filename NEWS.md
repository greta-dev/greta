...
* `nadam()` now takes slightly more iterations to reach the same answer, because Keras 3 implements Nadam differently to the Keras 2 optimiser greta used before (#633).
* `opt()` with a Keras optimiser (such as `adam()` or `gradient_descent()`) is faster, as the gradient step is now compiled with `tf_function()` rather than making several R to Python calls per iteration; this measured about 2.3 times faster on a small linear regression (#633).
