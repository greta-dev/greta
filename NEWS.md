...
* The managed (uv) backend now starts offline when its environment is already provisioned: greta enables uv's offline mode (`UV_OFFLINE`) once reticulate's uv cache is populated, so it no longer re-resolves against PyPI on every start and works on air-gapped machines; set `UV_OFFLINE` yourself to override (#814).
...
* `greta_remove()` consolidates greta's Python removal helpers behind a single `what` argument to remove the `greta-env-tf2` conda environment, miniconda, reticulate's uv cache, the stored preference, or all of them; `destroy_greta_deps()`, `greta_remove_all_deps()`, `remove_greta_env()`, `remove_miniconda()`, `remove_reticulate_uv_cache()`, `reinstall_greta_env()`, and `reinstall_miniconda()` are now deprecated (#814).
...
* `greta_set_python_path()` now accepts an environment directory (a virtualenv or conda prefix), not just a Python binary, looking for `bin/python` (or `Scripts/python.exe` on Windows) inside it, which eases offline and pre-provisioned setups (#814).
