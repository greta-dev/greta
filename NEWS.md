...
* greta supports TensorFlow 2.18.0 to 2.21.0, with TensorFlow Probability 0.25.0, and the documentation now says so plainly rather than implying that all three versions are a free choice. Two constraints produce that narrow band: greta's optimisers use the Keras 3 API, which arrives in TensorFlow 2.16, and TensorFlow Probability has had no release since 0.25.0 in November 2024, which is tested against TensorFlow 2.18. TensorFlow Probability is therefore settable but effectively fixed — the older releases pair with TensorFlow versions greta no longer supports (#633, #638).
* `greta_deps_spec()` now rejects a TensorFlow version that is too old, as well as one that is too new, and the error names the supported range and both reasons for it. TensorFlow 2.16 and 2.17 install without complaint and then fail to import TensorFlow Probability, so you would previously have found out after a full install rather than before it (#633, #638).
