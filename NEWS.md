...
* `mcmc()` no longer keeps the free state of every warmup draw. Those values were discarded the moment warmup ended and nothing read them in between, but they were accumulated with `rbind`, which copies everything gathered so far on each append, so the cost grew with the square of the number of bursts; at 200 free parameters and 2000 warmup iterations that came to over a gigabyte allocated in total, all of it thrown away. Draws are unchanged (#834).
