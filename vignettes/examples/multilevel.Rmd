...
occupancy <- matrix(rbinom(n_sites * n_species, 1, 0.5), nrow = n_sites) # create n_sites * n_species matrix of observed occupancy
```
</div>
...
g <- normal(0, 10, dim = c(n_env, n_traits)) # sub-model parameters are drawn from normal distributions
beta <-  g %*% t(U) # parameters of the base model are a function of the parameters of the sub-model

env_effect <- X %*% beta # transpose environmental matrix X for matrix multiplication
