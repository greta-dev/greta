...
for (id in unique(species_id)) {
  Sig <- diag(2)
  species_random_sd <- wishart(5, Sig)
  species_random_effect <- multivariate_normal(rep(0, 2), species_random_sd)
  mu[species_id == id] <- int + coef * iris$Sepal.Width[species_id == id] + Z[species_id == id,] %*% t(species_random_effect)
}
