...
```{r}
plan(multisession)
draws <- mcmc(e88_model, chains = 8, n_samples = 500, warmup = 250)
```
...
```{r}
bayesplot::mcmc_trace(draws, pars = c("b1", "b2"))
```
...
```{r}
bayesplot::mcmc_trace(draws, pars = paste0("a", 1:5))
```
...
```{r}
mcmc_areas(as_probs, pars = c("prob1", "prob2"))
```
