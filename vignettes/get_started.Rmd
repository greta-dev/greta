...
greta's [plotting functionality](#plotting) depends on the [DiagrammeR package](http://rich-iannone.github.io/DiagrammeR/). Because DiagrammeR depends on the [igraph](http://igraph.org/r/) package, which contains a large amount of code tthat needs to be compiled, DiagrammeR often takes a long time to install. So, DiagrammeR isn’t installed automatically with greta. If you want to ploti greta models, you can install igraph and DiagrammeR from CRAN.
...
**Tip 1**: igraph depends on [XML](https://cran.r-project.org/web/packages/XML/index.html) R package. On igraph's website, they inform you that you need to install <code>libxml2</code> library on Linux (and other Unix-like systems, but not on MacOSX). However, installing XML sometimes fails on MacOS too. Here's a workaround:

1. Install libxml2 using homebrew, with the command: `brew install libxml2`;
...
**Tip 2**: Installing igraph can be problematic with some linux installations, with `libgfortran.so.4` not being found. A workaround is to install gfortran-7 as described [here](https://askubuntu.com/questions/859256/how-to-install-gcc-7-or-clang-4-). 
