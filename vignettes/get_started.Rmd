...
Before you can fit models with greta, you will also need to have a working installation of Google's [TensorFlow](https://www.tensorflow.org/) python package (version 1.8.0 or higher). greta exports `install_tensorflow()` from the `tensorflow` R package, which will attempt to install the latest version of TensorFlow from within your R session.Moreover, you need to install an additional Python package called [tensorflow-probability](https://github.com/tensorflow/probability). [TensorFlow Probability](https://medium.com/tensorflow/introducing-tensorflow-probability-dca4c304e245) is a library for probabilistic reasoning and statistical analysis in TensorFlow.

Example to install Tensorflow version 1.8.0 :
...
**Tip 1**: If you have already installed `TensorFlow` with [Anaconda](www.anaconda.com/download), you can have some incompatibilities to install `tensorflow` R package. It is noted that the Anaconda installation is community supported, not officially supported by TensorFlow. You need to uninstall Anaconda before installing [Tensorflow](https://www.tensorflow.org/install/) via Virtualenv, "native" pip or Docker in order to be compatibile with the `tensorflow` R package.

**Tip 2**: When you install `virtualenv` Python library, even if virtualenv is successfully installed, you can have a warning message about two libraries, `nose<` and `tornado`, for `matplotlib` library. However, these two Python libraries are currently not used in greta, so you don't need to install them to use greta.

**Tip 3**: If you want to use probabilistic modeling and statistical inference in TensorFlow, you need to install [tfp-nightly](https://pypi.org/project/tfp-nightly/) and [tf-nightly](https://pypi.org/project/tf-nightly/). You may be need to define the version to use, such as:
...
**Tip 1**: igraph depends on [XML](https://cran.r-project.org/web/packages/XML/index.html) R package. On igraph's website, they inform you that you need to install <code>libxml2</code> library on Linux (and other Unix-like systems, but not on MacOSX). However, you can fail to install XML R package on MacOS. So, you need to:

1. Install libxml2 using the bash command: `brew install libxml2`;
2. Find where libxml2's configuration file is installed using the bash command: `type xml2-config`
3. Set environment variable for libxml2's configuration folder using the bash command: `export LIBXML_INCDIR=my_path_libxml2_configuration_folder` using the path from the previous command;
4. Instead to setting environment variable for libxml2's configuration folder using the bash command, you can set environment variable for libxml2's configuration file directly in R session using the R command: 
```{r define_XML_variable, eval = FALSE}
 Sys.setenv(XML_CONFIG="my_path_libxml2_configuration_file")
```
5. Install to the appropriate XML R package and then igraph
```{r install_XML, eval = FALSE}
install.packages('XML')
```

**Tip 2**: You can have some issues to install igraph on Ubuntu 16.04 (Issue with libgfortran.so.4 not found). Workaround is to install gfortran-7 as described [here](https://askubuntu.com/questions/859256/how-to-install-gcc-7-or-clang-4-). 

<hr>

### future
To be able to have parallel processing of greta R expressions, you need to install [Future](https://cran.r-project.org/web/packages/future/index.html) R package and use this R expression
```{r multiple_Session, eval = FALSE}
plan(multisession)
```

If you want to evaluate R expressions asynchronously, you can install future from CRAN.
```{r install_future, eval = FALSE}
...
**Tip 1**: If you modify your model, you need to clear a bunch of tf objects without restarting R using R command 
```{r clear_tf_objects, eval = FALSE}
