...
When [`model()`](https://greta-dev.github.io/greta/reference/model.md)
is called, that inheritance information is used to construct the
directed acyclic graph (DAG) that defines the model. The inheritance
also preserves intermediate nodes, such as those creates in multi-part
...
user runs
[`model()`](https://greta-dev.github.io/greta/reference/model.md), which
creates a ‘dag_class’ object to store the relevant nodes, the
