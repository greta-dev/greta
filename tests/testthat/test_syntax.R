...
test_that("`distribution<-` errors informatively", {
  ...
  expect_snapshot(error = TRUE, distribution(y) <- x)

  expect_snapshot(error = TRUE, distribution(y) <- as_data(x))

  # no density on the right
  expect_snapshot(error = TRUE, distribution(y) <- variable())
  ...
  expect_snapshot(error = TRUE, distribution(y_) <- normal(0, 1))
  ...
  d <- normal(0, 1)
  distribution(y1) <- d
  expect_snapshot(error = TRUE, distribution(y2) <- y1)

  # assignment to a variable
  z <- variable()
  expect_snapshot(error = TRUE, distribution(z) <- normal(0, 1))

  # assignment to an op
  z2 <- z^2
  expect_snapshot(error = TRUE, distribution(z2) <- normal(0, 1))
