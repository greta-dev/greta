...
# install timeout message is captured
    ...
    Output
      ...
      Then:
      `reticulate::py_install( packages = c( 'numpy', 'tensorflow==2.21.0', 'tensorflow-probability==0.25.0', 'tf-keras==2.21.0' ), envname = 'greta-env-tf2', pip = TRUE )`
...
# install failure message is captured
    ...
    Output
      ...
      `library(reticulate)`
      `py_require(packages = c('tensorflow==2.21.0', 'tensorflow-probability==0.25.0', 'tf-keras==2.21.0'), python_version = '3.12')`
      `py_require()`
      `py_config()`
