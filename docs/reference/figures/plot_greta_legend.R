...
export_graph(gr1, file_name = f_nodes, width = 1005, height = 249)

ns <- 0.01
nodes2 <- create_node_df(
  n = 4,
  label = '',
  type = "lower",
  alpha = 0,
...
export_graph(gr2, file_name = f_edges, width = 631, height = 249)
...
dim_pixels <- c(width = round(1005 + 631 + 1005 / 16), height = 249)

dim_inches <- dim_pixels / 10

# layout with a gap inbetween
mat <- matrix(rep(c(1, 2, 3), c(16, 1, 10)), nrow = 1)
