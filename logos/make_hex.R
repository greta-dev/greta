...
par(pty = "s", xpd = NA, bg = "black", mar = rep(0, 4), oma = rep(0, 4))
plot.new()
...
set.seed(2018 - 02 - 20)
dim <- mask_dim * 2
cols <- c(greta:::greta_col("lighter"), greta:::greta_col("main"))
ramp_cols <- colorRampPalette(cols)(2000) #[-(1:1000)]

png("logos/hex_bg.png", width = dim[1], height = dim[2], pointsize = 30)
tesselation_image(
  ncol = dim[1],
  nrow = dim[2],
  max_edge = 0.1,
...
par(pty = "s", xpd = NA, bg = "black", mar = c(5, 4, 4, 2) + 0.1)
